...
def print_death_age(people_dict):
    ...
    for key, value in people_dict.items():
        ...
        print(f' {name} {death_year - birth_year} > {birth_year}> {death_year}')
    """
    Parameter
...
def print_marriages(marriages_dict, people_dict):
    ...
    for key, value in marriages_dict.items():
        husband_detail = value[HUSBAND_KEY_INDEX]
        wife_detail = value[WIFE_KEY_INDEX]
        wedding_year = value[WEDDING_YEAR_INDEX]
     
    
    #The name and age in the wedding year of the husband
        if husband_detail in people_dict:
            detail = people_dict[husband_detail]
            husband_name = detail[NAME_INDEX]
            husband_birth_year = detail[BIRTH_YEAR_INDEX]
        husband_age_at_marriage = wedding_year - husband_birth_year
        
    #The name and age in the wedding year of the wife
        if wife_detail in people_dict:
            detail = people_dict[wife_detail]
            wife_name = detail[NAME_INDEX]
            wife_birth_year = detail[BIRTH_YEAR_INDEX]     
        wife_age_at_marriage = wedding_year - wife_birth_year

        print(f'{husband_name} {husband_age_at_marriage} > {wedding_year} > {wife_name} {wife_age_at_marriage}')
    """
    Parameters
